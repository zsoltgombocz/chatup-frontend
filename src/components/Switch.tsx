...
function Switch({ options, onChange = undefined, className }: Props) {
    ...
    return (
        <div className={'border border-1  border-gray-600 flex flex-row rounded-full w-fit p-1 relative ' + className}>
            ...
            {
                options.map((opt, i) =>
                    <div className={'z-10 px-2 py-1 flex text-gray-600 rounded-full justify-center items-center gap-1 w-24 cursor-pointer'} onClick={() => handleOnClick(i)}>
                        <span>{opt.icon}</span>
                        {selectedIndex === i ? <span className={"font-medium mr-1"}>{opt.text}</span> : <></>}
                    </div>
                )
            }
        </div>
